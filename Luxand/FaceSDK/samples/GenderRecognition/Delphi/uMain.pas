...
var
  ...
  ConfidenceMale, ConfidenceFemale: single;
  s: string;
  TextWidth: integer;
  ...
  begin
    ...
    begin
      ...
      image1.Canvas.Rectangle(left, top, right, bottom);
	    ...
	    if (0 = FSDK_GetTrackerFacialAttribute(tracker, 0, IDs[i], 'Gender', @AttributeValues, sizeof(AttributeValues))) then
      begin
        ConfidenceMale := 0;
        ConfidenceFemale := 0;
        ...
        begin
          str(round(100*ConfidenceMale), s);
          s := 'Male, ' + s + '%';
        end
        else
        begin
          str(round(100*ConfidenceFemale), s);
          s := 'Female, ' + s + '%';
        end;
